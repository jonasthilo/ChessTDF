...
/**
 * PathAdjacent strategy: places towers on rows 3 and 6 (adjacent to the path).
 * Fills these rows first, prioritizing center columns.
 * Mixes tower types evenly. Upgrades existing towers to max before building new.
 */
export class PathAdjacentStrategy implements Strategy {
  ...
  decideActions(
    ...
    // Phase 1: Upgrade existing towers to max level first
    ...
    // Phase 2: Build new towers on adjacent rows, center columns first
